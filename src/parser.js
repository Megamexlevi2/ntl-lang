...
class Parser {
  ...
  parseStmt() {
    ...
    if (t.type===TokenType.KEYWORD) {
      switch(t.value) {
        ...
        case 'val':case 'const': {
          // const require(ntl, ...) is an NTL require statement
        ...
        // 'const' handled above in val/const case
        ...
        case 'log': {
          // 'log.info(...)' — log used as identifier (member access)
  ...
  parseFnDecl(isAsync, opts) {
    opts = opts || {};
    const t=this.peek();
    // Don't eat 'fn' if already eaten by caller (when opts.fnAlreadyEaten)
  ...
  parseFnParams() {
    ...
    while(!this.check(TokenType.PUNCTUATION,')')){
      ...
      // Destructuring params: fn foo({a, b}) or fn foo([a, b])
  ...
  parseClassMember() {
    ...
    // 'state' keyword = instance field (readable shorthand for class property)
  ...
  parseIf() {
    ...
    if(this.checkKw('elif')){
      // elif: synthesize as else-if without consuming a keyword via parseIf
      const et=this.advance(); // consume 'elif'
      ...
      if(this.checkKw('elif')){
        ...
        // Instead, just recursively build the elif chain
      }
      // build elif chain manually
      let chain=null;
  ...
  parseEach() {
    // each item in list { } — clean alias for for...of
    const t=this.advance();
    let id=null, destructure=null;
    if(this.check(TokenType.PUNCTUATION,'{')) destructure=this.parseObjDestructure();
    else if(this.check(TokenType.PUNCTUATION,'[')) destructure=this.parseArrDestructure();
  ...
  parseGuard() {
    // guard expr else { } — if expr is falsy, run else block (which should return/throw)
    const t=this.advance();
  ...
  parseDefer() {
    // defer { } — run block at end of scope (using try/finally)
    const t=this.advance();
    const body=this.parseBlock();
  ...
  parseLog() {
    // log expr, expr, ... — shorthand for console.log
    const t=this.advance();
    const args=[];
  ...
  parseTry() {
    const t=this.advance();
    // try? expr — safe try expression (returns null on error)
    if(this.check(TokenType.OPERATOR,'?')) {
  ...
  parseExprAsBlock() {
    const t=this.peek();
    // Allow statement-level keywords inside match arms
  ...
  parseNTLRequire() {
    const t=this.peek();
    // skip 'require' keyword if we're on it (called after advancing past const/val)
  ...
  parseIfHave() {
    ...
    const consequent=this.parseBlock(); let alternate=null;
    if(this.eatIf(TokenType.KEYWORD,'else')) alternate=this.parseBlock();
    return new ASTNode('IfHaveStmt',{expr,alias,consequent,alternate,id},t.line,t.col);
  }
  parseIfSet() {
    const t=this.advance();
    const id=this._ifsetCounter++;
  ...
  parseExprStatement() {
    ...
    if(decorators.length>0) {
      // @class Name { }  — NTL enhanced class syntax
      // If the last decorator is named 'class' and next token is an identifier, parse as class
      const lastDec=decorators[decorators.length-1];
      if(lastDec&&lastDec.name==='class') {
        // @class Name { } — enhanced class syntax
        // 'class' was consumed as decorator name, so we parse the class body directly
        const cleanDecs=decorators.slice(0,-1);
        const tk=this.peek();
        let name=(this.check(TokenType.IDENTIFIER)||this.check(TokenType.KEYWORD))?this.advance().value:null;
        const typeParams=this.parseTypeParams();
        let superClass=null,interfaces=[];
        ...
        this.eat(TokenType.PUNCTUATION,'}');
        return new ASTNode('ClassDecl',{name,superClass,interfaces,typeParams,members,isAbstract:false,decorators:cleanDecs},tk.line,tk.col);
      }
      let inner;
      if(this.checkKw('fn')) {
  ...
  parseDecorators() {
    ...
    while(this.check(TokenType.OPERATOR,'@')) {
      this.advance();
      // Allow @class, @abstract etc (keywords as decorator names)
      ...
      switch(t.value) {
        ...
        case 'async': {
          if(this.peek(1).value==='fn') {
            this.advance(); this.advance(); // consume 'async' and 'fn'
          ...
          // async (params) => body  OR  async x => body
        ...
        case 'have': {
          this.advance(); const value=this._parsePostfix(this.parsePrimary());
          return new ASTNode('HaveExpr',{value},t.line,t.col);
        }
        // Keywords as identifiers only when used as member access (log.info, etc.)
        // When followed by '(' they are handled by their specific cases below
        case 'error': {
          this.advance();
          return new ASTNode('Identifier',{name:t.value},t.line,t.col);
        }
        case 'log': {
          // log.info(...) => identifier, but log(...) => LogExpr handled at stmt level
          this.advance();
          return new ASTNode('Identifier',{name:t.value},t.line,t.col);
        }
        case 'assert': {
          // assert can appear as identifier in expressions
          this.advance();
          return new ASTNode('Identifier',{name:t.value},t.line,t.col);
        }
        case 'Not': {
        ...
        case 'match': {
          // match as expression: val result = match x { case ... }
          return this.parseMatch();
        }
        case 'fn': {
          // fn as expression (inline function)
          return this.parseFnDecl(false);
        }
        case 'async': {
          // async fn as expression
          if(this.peek(1).value==='fn') { this.advance(); return this.parseFnDecl(true); }
          break;
        }
        case 'new': {
          // already handled above, just break to avoid fallthrough
          break;
        }
      }
    }
