...
const BUILTINS = new Set([
  ...
  '__ntl','_ntl','__ntlRequire','__ntlDir','arguments','eval','AggregateError',
  'range','sleep','log','assert',
  'class','singleton','sealed','abstract','memo','deprecated',
  'timeout','retry','bind','validate','typed','event','cache'
]);
class Scope {
...
class ScopeAnalyzer {
  ...
  _hoistAll(stmts) {
    for (const s of stmts) {
      ...
      if (s.type === 'ComponentDecl' && s.name) this.declare(s.name, { kind: 'fn', line: s.line });
  ...
  visitStmt(node) {
    ...
    switch (node.type) {
      ...
      case 'ComponentDecl': this._visitFn(node); break;
      ...
      case 'RepeatStmt': { if (node.count) this.visitExpr(node.count); this.push('block'); this.visitBlock(node.body); this.pop(); break; }
      case 'GuardStmt': { this.visitExpr(node.test); this.push('block'); this.visitBlock(node.alternate); this.pop(); break; }
      case 'DeferStmt': { this.push('block'); this.visitBlock(node.body); this.pop(); break; }
      case 'LogStmt': { for (const a of (node.args || [])) this.visitExpr(a); break; }
      case 'AssertStmt': { this.visitExpr(node.test); if (node.message) this.visitExpr(node.message); break; }
      case 'SleepStmt': { this.visitExpr(node.ms); break; }
      case 'SelectStmt': { for (const c of (node.cases || [])) { this.visitExpr(c.channel); if (c.body) this.visitStmt(c.body); } break; }
      case 'DecoratedExpr': {
        if (node.decorators) {
  ...
  _visitFn(node) {
    ...
    for (const p of (node.params || [])) {
      ...
      if (p.destructure) this._declareDestructure(p.destructure);
    }
  ...
  visitExpr(node) {
    ...
    switch (node.type) {
      ...
      case 'ArrowFn': case 'FnExpr': {
        ...
        if (node.body) {
          if (node.body.type === 'Block') { this._hoistAll(node.body.body || []); this.visitBlock(node.body); }
          else {
            const STMT_TYPES = new Set(['LogStmt','IfStmt','WhileStmt','ForStmt','ReturnStmt','ThrowStmt','ExprStmt','VarDecl','FnDecl','ClassDecl']);
            if (STMT_TYPES.has(node.body.type)) this.visitStmt(node.body);
      ...
      case 'NotExpr': if (node.value) this.visitExpr(node.value); break;
      case 'TrySafeExpr': if (node.expr) this.visitExpr(node.expr); break;
      case 'RangeExpr': for (const a of (node.args || [])) this.visitExpr(a); break;
      case 'SleepExpr': if (node.ms) this.visitExpr(node.ms); break;
      case 'NaxImportExpr': case 'RequireExpr': case 'NTLRequireExpr': case 'ChannelExpr': break;
      default: break;
    }
  }
}
