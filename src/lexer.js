...
class Lexer {
    ...
    while (this.peek() !== q && this.pos < this.source.length) {
      ...
      if (this.peek() === '\\') {
        ...
        if (e === 'u') {
          ...
          let hex=''; for (let i=0;i<2;i++) hex+=this.advance(); raw+=String.fromCharCode(parseInt(hex,16));
  ...
  _canBeRegex() {
    // A '/' is a regex if the previous non-whitespace token is an operator, keyword, or punctuation
    // (i.e., not after an identifier, number, string, ), or ]
    if (this.tokens.length === 0) return true;
    const prev = this.tokens[this.tokens.length - 1];
    if (!prev) return true;
    if (prev.type === TokenType.IDENTIFIER) return false;
    if (prev.type === TokenType.NUMBER) return false;
    if (prev.type === TokenType.STRING) return false;
    if (prev.type === TokenType.REGEX) return false;
    if (prev.type === TokenType.PUNCTUATION && (prev.value === ')' || prev.value === ']')) return false;
    return true;
  }
  readRegex() {
    const sl = this.line, sc = this.col;
    ...
    while (this.pos < this.source.length) {
      const ch = this.peek();
      if (ch === '\\') { pattern += this.advance() + this.advance(); continue; }
      if (ch === '[') { inClass = true; pattern += this.advance(); continue; }
      if (ch === ']') { inClass = false; pattern += this.advance(); continue; }
      if (ch === '/' && !inClass) break;
      if (ch === '\n') { // Not a regex after all - rewind
        this.pos = sc - 1; this.line = sl; this.col = sc;
        return null;
      }
      pattern += this.advance();
    }
    if (this.peek() !== '/') return null; // Not a valid regex
    this.advance(); // consume closing '/'
    let flags = '';
    while (/[gimsuy]/.test(this.peek())) flags += this.advance();
    return new Token(TokenType.REGEX, { pattern, flags }, sl, sc);
  }
  tokenize() {
    while (this.pos < this.source.length) {
      this.skipWhitespace();
      ...
      // Regex literal — only in expression-start positions
      if (ch === '/' && this._canBeRegex()) {
        const re = this.readRegex();
        if (re) { this.tokens.push(re); continue; }
      }
